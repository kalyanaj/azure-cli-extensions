...
def cli_eventgrid_event_subscription_create(   # pylint: disable=too-many-locals
    ...
    if preferred_batch_size_in_kilobytes is not None:
        ...
        if preferred_batch_size_in_kilobytes > 1024:
            raise CLIError('usage error: preferred_batch_size_in_kilobytes must be a number between 1 and 1024.')

    destination = _get_endpoint_destination(endpoint_type, endpoint, max_events_per_batch, preferred_batch_size_in_kilobytes, aad_application_object_id)
...
def update_event_subscription(
    ...
    if endpoint is not None:
        event_subscription_destination = _get_endpoint_destination(
            ...
            event_subscription_destination.azure_active_directory_application_object_id)
...
def _get_endpoint_destination(endpoint_type, endpoint, max_events_per_batch, preferred_batch_size_in_kilobytes, aad_application_object_id):
    if endpoint_type.lower() == WEBHOOK_DESTINATION.lower():
        destination = WebHookEventSubscriptionDestination(
            ...
            azure_active_directory_application_object_id=aad_application_object_id)
...
def _get_event_delivery_schema(event_delivery_schema):
    ...
    elif event_delivery_schema.lower() == CLOUDEVENTV01SCHEMA.lower():
        raise CLIError('usage error: CloudEvent V 0.1 is not supported anymore. It has been superseded by CloudEvent 1.0 Schema. '
                       '--event-delivery-schema supported values are'
                       ':' + EVENTGRID_SCHEMA + ',' + CUSTOM_INPUT_SCHEMA +
                       ',' + CLOUDEVENTV1_0_SCHEMA)
    else:
...
def _get_input_schema_and_mapping(
    ...
    # NO LONGER SUPPORTED
    elif input_schema.lower() == CLOUDEVENTV01SCHEMA.lower():
        raise CLIError('usage error: CloudEvent V 0.1 is not supported anymore. It has been superseded by CloudEvent 1.0 Schema. '
